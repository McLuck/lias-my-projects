					...
					<table width="100%" class="display" id="tableLocais">
						...
						<tbody>
							<c:forEach items="${praca.localizacoes}" var="lcs">
								<c:if test="${lcs!=null}">
									<tr id="tr_loc_${lcs.id}" onclick="onClickInitEditLocal(${lcs.id});" onmouseover="closeAll(event);"<%i++;out.print((i % 2 ==0)?"class='even'":"class='odd'");%>>
										<td width="92">${lcs.numero}</td>
										<td width="498">${lcs.descricao}</td>
										<td width="199" id="labelSentidosPraca${lcs.id}">
											${lcs.siglas}</td>
									</tr>
									<tr style="display: none;" id="edit_loc_${lcs.id}" onmouseover="viewIconsLocais(${lcs.id});" onmouseout="hideIconsLocais(${lcs.id});"<% out.print((i % 2 ==0)?"class='even2'":"class='odd2'");%>>
