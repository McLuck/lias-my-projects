...
public class TabelaDAO extends BaseDAO<Tabela> {
    ...
    private String getSelectFuncionalidades(int vid, String[] ids) {
        StringBuffer sb = new StringBuffer();
        for(String s : ids)sb.append(s.trim().concat(", "));
        String aux = sb.toString();
        if (aux.trim().endsWith(", ")) {
            aux = aux.trim().substring(0, aux.trim().length() - 2);
        }
    ...
    private String getSelectFuncionalidadesIN(int vid, String ids){
        ids = ids.replace(" ", "").trim().replace(",,", ",");
        StringBuffer sb = new StringBuffer();
        sb.append("select t.id as id, t.nome, t.ordem as ordem, cqry.nomecampo, cqry.valor as valor from tcampo cFiltro, ttabela t, tcampo cqry where t.id = cFiltro.tabela_id and t.id = cqry.tabela_id and cFiltro.nomecampo = \"idfuncionalidade\" AND  cFiltro.valor in (");
        sb.append(ids);
        sb.append(") AND t.versao_id = ");
        sb.append(vid);
        sb.append(";");
    ...
    private String tratarIds(String input){
    ...
    public List<Tabela> getFuncionalidades(int vid, String ids){
        List lista = getSession().createSQLQuery(getSelect(vid, tratarIds(ids))).list();
        return tratarSelectFuncionalidades(lista);
    }

    public List<Tabela> getFuncionalidadesIN(int vid, String ids){
    	List lista = getSession().createSQLQuery(getSelectFuncionalidadesIN(vid, tratarIds(ids))).list();
    	return tratarSelectFuncionalidades(lista);
    }
