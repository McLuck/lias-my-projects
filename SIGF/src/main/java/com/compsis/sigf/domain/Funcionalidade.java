...
import com.compsis.sigf.dao.FuncionalidadeAcessoDAO;
import com.compsis.sigf.dao.PapelFuncionalidadeDAO;
 ...
 * @author Lucas Israel
 */
public class Funcionalidade extends DomainObject implements Replicable, SqlInterface {
    ...
    public String toSqlInsert(Versao v) {
        if (funcionalidades != null) {
            if (!funcionalidades.toUpperCase().trim().contains("NA")) {
                TabelaDAO tdao = (TabelaDAO) AFactoryDao.getInstance(TabelaDAO.class);
                List<Tabela> tlist = new ArrayList<Tabela>();
                String vt[] = funcionalidades.split(",");
                for (String s : vt) {
                    List<Tabela> listaT = tdao.getTabelas(v.getId(), "funcionalidade", "idfuncionalidade", s);
                    tlist.addAll(listaT);
                }
                ...
                List<PapelFuncionalidade> listaP = getPapelFuncionalidades();
                if(listaP!=null){
                    for(PapelFuncionalidade pf : listaP){
                        if(pf!=null){
                            script = script.concat(pf.toSqlInsert(v));
                        }
                    }
                }
                List<FuncionalidadeAcesso> listaF = getFuncionalidadeAcessos();
                if(listaF!=null){
                    for(FuncionalidadeAcesso fa : listaF){
                        if(fa!=null){
                            script = script.concat(fa.toSqlInsert(v));
                        }
                    }
                }
                return script;
